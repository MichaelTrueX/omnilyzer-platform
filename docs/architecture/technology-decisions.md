...
| Backend | Python + Django 6 | ACCEPTED DIRECTION | Task 002 validated Django as the core backend foundation with PostgreSQL, transactions, migrations, application-service separation, security extension points, admin, OpenAPI adapters, and horizontally scalable topology. | Validate remaining tenancy/RLS, authentication, deployment, background-work, and production-scale concerns independently; routine Django version upgrades remain subject to regression testing. |
...
Each validated major choice must be recorded through the [`ADR process`](../adr/README.md); a successful spike does not change status by itself. The accepted backend and API direction is recorded in [ADR 0001](../adr/0001-backend-framework-and-api-adapter.md).
