...
The consumer downloads the exact wheel and npm tarball without installation or import, pulls but never runs the OCI image, and retrieves the exact evidence archive and its separate signature bundle with the short-lived read-only Cloudsmith identity. It verifies the archive signature before safe extraction, requires the exact twelve-file allowlist, verifies the evidence manifest and every direct blob/OCI signature against the exact identity and issuer, and validates all hashes, CycloneDX versions, SLSA statement fields, Git source dependency, and immutable OCI digest. Only after every cryptographic and metadata check passes does it install and import the already-downloaded local wheel and npm tarball with lifecycle scripts disabled. The order is therefore download, cryptographic verification, provenance/SBOM validation, then code execution.
