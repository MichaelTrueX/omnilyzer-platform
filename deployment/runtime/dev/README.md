...
Compose project `omnilyzer-task014-dev` has exactly `canary-blue`, `canary-green`, and `deployment-nginx`, and no build directive. `CANARY_IMAGE` must be exactly `oci-dev.omnilyzer.ai/omnilyzer/task013-release-canary@sha256:<64 lowercase hex>`. The adapter rejects tags, alternate origins/repositories, multiple values, malformed digests, and whitespace, then verifies the exact single local RepoDigest before candidate start. PR B neither pulls nor runs release 0.14.2.
...
The audit sink writes canonical one-record-per-line JSON to `/var/log/omnilyzer/deployment/dev/events.jsonl`, modes 0700/0600, using serialized `O_APPEND` and fsync. It rejects malformed, partial, noncanonical, duplicate, oversized (over 16 KiB), or chain-invalid history. `previous_event_sha256` links full records. This is tamper-evident and owner-controlled, not root-tamper-resistant; remote/WORM audit is deferred.
