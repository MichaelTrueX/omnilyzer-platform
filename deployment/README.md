...
`execution.py` remains unchanged and defines immutable closed runtime, no-secret, ingress, and executor-request models. The only operation is `deploy` to DEV. Canonical JSON reuses the repository's sorted, compact, ASCII, newline-terminated representation. Broker, privileged-executor, and transport types remain protocols. C1 adds no HTTP or Unix server, systemd service, or public endpoint.
